...
let get_suff sch_type sch_sort =
  try
    fst (
      ...
      | None -> Hashtbl.find scheme_object_table (sch_type,Some (UnivGen.QualityOrSet.Qual (Sorts.Quality.QConstant QType)),true)
      )
  with Not_found ->
  (try
    fst (match sch_sort with
      | Some st -> Hashtbl.find scheme_object_table (sch_type,sch_sort,false)
      | None -> Hashtbl.find scheme_object_table (sch_type,Some (UnivGen.QualityOrSet.Qual (Sorts.Quality.QConstant QType)),false)
       )
...
let redeclare_schemes eff =
  ...
  in
  let schemes = Cmap.fold fold eff.Evd.seff_roles UnivGen.Map.empty in
...
and define_mutual_scheme ?locmap kind ~internal names inds eff =
  ...
  let kind = match sortf with Some k -> (strl,sortf,true) | None -> (strl,Some (UnivGen.QualityOrSet.Qual (Sorts.Quality.QConstant QType)),true) in 
  match Hashtbl.find scheme_object_table kind with
...
let define_individual_scheme ?loc ?(intern=false) kind names ind =
  match define_individual_scheme ?loc kind ~internal:intern names ind Evd.empty_side_effects with
    | None -> ()
    | Some (cst,eff) -> redeclare_schemes eff
